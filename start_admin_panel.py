...
def start_admin_panel():
    ...
    try:
        import requests
        response = requests.get("http://172.31.0.2:5000/api/admin/users?admin_user_id=admin_001&limit=1", timeout=5)
    ...
    try:
        ...
        try:
            import requests
            response = requests.get("http://172.31.0.2:5000/api/admin/users/reading-progress?admin_user_id=admin_001&limit=1000", timeout=10)
            if response.status_code == 200:
                data = response.json()
                if data.get('success'):
