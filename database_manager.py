...
class DatabaseManager:
    ...
    def get_or_create_active_session(self, user_id: str, reuse_recent: bool = False, timeout_minutes: int = 30) -> str:
        ...
        if not reuse_recent:
            logger.info(f"ℹ️ 创建新对话session，为用户 {user_id} 创建新session")
                ...
                with self.connection.cursor(pymysql.cursors.DictCursor) as cursor:
                    ...
                    if result and result['session_id']:
                        ...
                        if time_diff <= timeout_minutes:
                            ...
                            logger.info(f"✅ 复用最近的session: {session_id} (距离上次交互 {time_diff:.1f} 分钟)")
                            return session_id
                        else:
                            # 超过超时时间，创建新session
                            logger.info(f"ℹ️ 距离上次交互 {time_diff:.1f} 分钟，超过 {timeout_minutes} 分钟，创建新session")
                            return self.create_session(user_id)
                    
                    # 如果没有找到任何交互记录，创建新session
                    logger.info(f"ℹ️ 未找到历史交互记录，为用户 {user_id} 创建新session")
                    return self.create_session(user_id)
            ...
            try:
                ...
                with self.connection.cursor(pymysql.cursors.DictCursor) as cursor:
                    ...
                    if not result or not result.get('timestamp'):
                        ...
                        logger.info(f"ℹ️ session {session_id} 不存在或无交互记录，创建新session")
                        return self.create_session(user_id)
                    ...
                    if time_diff > timeout_minutes:
                        ...
                        logger.info(f"ℹ️ session {session_id} 距离上次交互 {time_diff:.1f} 分钟，超过 {timeout_minutes} 分钟，创建新session（新对话）")
                        return self.create_session(user_id)
                    else:
                        # 在超时时间内，使用原session（同一对话的继续）
                        logger.info(f"ℹ️ session {session_id} 有效，距离上次交互 {time_diff:.1f} 分钟，继续使用")
                        return session_id
        ...
        for attempt in range(max_retries):
            try:
                ...
                with self.connection.cursor(pymysql.cursors.DictCursor) as cursor:
                    ...
                    logger.info(f"ℹ️ 检查session {session_id}: {'存在' if exists else '不存在'}")
                    return exists
            ...
            try:
                ...
                if force_new_session or not session_id or session_id.strip() == '':
                    ...
                    if not session_id:
                        ...
                        return False, None
                    logger.info(f"ℹ️ 创建新session: {session_id}")
                else:
                    # 直接使用提供的session_id，不做任何验证或复用检查
                    logger.info(f"ℹ️ 使用提供的session: {session_id}")
                ...
                with self.connection.cursor() as cursor:
                    ...
                    self.connection.commit()
                    logger.info(f"✅ 交互记录成功: {interaction_type}, session_id: {session_id}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                with self.connection.cursor(pymysql.cursors.DictCursor) as cursor:
                    ...
                    logger.info(f"✅ 获取session交互记录成功: {session_id}, 共 {len(results)} 条")
                    return results
            except Exception as e:
