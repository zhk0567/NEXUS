...
class SystemLauncher:
    ...
    def start_backend_server(self):
        ...
        try:
            ...
            for i in range(30):  # 最多等待30秒
                try:
                    response = requests.get("http://172.31.0.2:5000/api/health", timeout=2)
    ...
    def check_backend_status(self):
        """检查后端服务器状态"""
        try:
            response = requests.get("http://172.31.0.2:5000/api/health", timeout=5)
    ...
    def get_system_status(self):
        ...
        try:
            # 获取用户统计
            users_response = requests.get("http://172.31.0.2:5000/api/admin/users?admin_user_id=admin_001&limit=1", timeout=5)
            users_data = users_response.json() if users_response.status_code == 200 else {}
            
            # 获取阅读进度统计
            progress_response = requests.get("http://172.31.0.2:5000/api/admin/users/reading-progress?admin_user_id=admin_001&limit=1000", timeout=5)
    ...
    def display_system_info(self):
        ...
        print(f"🌐 后端地址: http://172.31.0.2:5000")
